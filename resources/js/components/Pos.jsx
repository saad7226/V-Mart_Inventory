...
export default function Pos() {
    ...
    const getProducts = useCallback(
        async (search = "", page = 1, barcode = "") => {
            setLoading(true);
            try {
                const res = await axios.get(`${window.APP_URL}/admin/get/products`, {
    ...
    const getCarts = async () => {
        try {
            const res = await axios.get(`${window.APP_URL}/admin/cart`);
            const data = res.data;
            setTotal(data?.total);
    ...
    function addProductToCart(id) {
        axios
            .post(`${window.APP_URL}/admin/cart`, { id })
            .then((res) => {
    ...
    function cartEmpty() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios
                    .put(`${window.APP_URL}/admin/cart/empty`)
                    .then((res) => {
                        setCartUpdated(!cartUpdated);
                        playSound(SuccessSound);
                        toast.success(res?.data?.message);
    ...
    function orderCreate() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios
                    .put(`${window.APP_URL}/admin/order/create`, {
                    ...
                    .then((res) => {
                        ...
                        window.location.href = `${window.APP_URL}/admin/orders/pos-invoice/${res?.data?.order?.id}`;
                    })
                    .catch((err) => {
                        toast.error(err.response.data.message);
                    });
                            ...
                            <div className="row">
                                ...
                                <div className="col">
                                    <button
                                        ...
                                        type="button"
                                        className="btn btn-primary btn-block text-white text-bold"
                                    >
                                        Checkout
                                    </button>
                                </div>
                                            ...
                                            <div className="text-center">
                                                <img
                                                    ...
                                                    onError={(e) => {
                                                        ...
                                                        e.target.src = `${window.APP_URL}/assets/images/no-image.png`;
                                                    }}
                                                    width={120}
                                                    height={100}
                                                />
