...
class UserManagementController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($users)
                ...
                ->addColumn('action', function ($data) {
                    ...
                    <a class="btn btn-sm bg-gradient-primary" href="'.route('backend.admin.user.edit', $data->id).'">
                        <i class="fas fa-edit"></i> Edit
                    </a>
                    <a class="btn btn-sm bg-gradient-danger" href="'.route('backend.admin.user.delete', $data->id).'" onclick="return confirm(\'Are you sure ?\')">
                        <i class="fas fa-trash-alt"></i> Delete
                    </a>';

                    if ($data->is_suspended) {
                        $html .= '<a class="btn btn-sm bg-gradient-success" href="'.route('backend.admin.user.suspend', ['id' => $data->id, 'status' => 0]).'">
                            <i class="fas fa-check-square"></i> Activate
                        </a>';
                    } else {
                        $html .= '<a class="btn btn-sm bg-gradient-warning" href="'.route('backend.admin.user.suspend', ['id' => $data->id, 'status' => 1]).'" onclick="return confirm(\'Are you sure ?\')">
                            <i class="far fa-times-circle"></i> Suspend
                        </a>';
                    }

                    if ($data->is_approved) {
                        $html .= '<a class="btn btn-sm bg-gradient-secondary" href="'.route('backend.admin.user.approve', ['id' => $data->id, 'status' => 0]).'" onclick="return confirm(\'Revoke approval ?\')">
                            <i class="fas fa-ban"></i> Revoke
                        </a>';
                    } else {
                        $html .= '<a class="btn btn-sm bg-gradient-info" href="'.route('backend.admin.user.approve', ['id' => $data->id, 'status' => 1]).'" onclick="return confirm(\'Approve this user ?\')">
                ...
                ->rawColumns(['thumb', 'created', 'action', 'suspend', 'approval', 'roles'])
                ->toJson();
        }
    ...
    public function approve($id, $status)
    {
        ...
        if ($user->is_approved == $status) {
            return back()->with('error', 'User approval status is already set');
        } else {
            ...
            if ($status == 1) {
                ...
                Mail::to($user->email)->send(new AccountApprovedMail($mailData));
            }
