...
class AuthController extends Controller
{
    ...
    public function login(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            if (!$user->is_approved) {
                return redirect()->back()->with('error', 'Your account is pending Super Admin approval.');
            }
    ...
    public function register(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            try {
                ...
                // 2. Create the Store automatically (named after the owner)
                $store = \App\Models\Store::create([
                    'name'     => $request->name . "'s Store",
                    'owner_id' => $newUser->id,
                ]);

                // 3. Link store_id back to user
                $newUser->store_id = $store->id;
                $newUser->save();

                // 4. Assign Admin role (store owner)
                ...
                return redirect()
                    ->route('login')
                    ->with('success', 'Welcome! Your store "' . $store->name . '" has been created. Your account is pending Super Admin approval. You will receive an email once approved.');
